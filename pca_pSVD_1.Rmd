...
To ensure `pca_pSVD` produces reliable and consistent results across different platforms and environments, a test suite was developed using the `testthat` framework. The complete test file is available at: [test-pca_pSVD.R](https://github.com/dereklei12/pca_R/blob/main/test/testthat/test-pca_pSVD.R).
...
All tests passed successfully, confirming that `pca_pSVD` is robust, reliable, and compatible with existing R PCA implementations. Continuous integration testing across multiple platforms (Ubuntu, Windows, macOS) is automated via [GitHub Actions](https://github.com/dereklei12/pca_R/actions).
