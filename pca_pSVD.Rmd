...
ncell = 300          # choose number of cells, do not go above 2000
...
run_pca_asym <- function(X, k) {
  pca_asym(X, ncomp = k, center = TRUE, scale = TRUE)
}

run_pca_em_wd <- function(X, k) {
  pca_em_woodbury(X, ncomp = k, center = TRUE, scale = TRUE)
}
```
...
bench_one <- function(k) {
  if (have_bench) {
    b <- bench::mark(
      ...
      pca_asym   = { run_pca_asym(data2, k) },
      pca_em_wd  = { run_pca_em_wd(data2, k)},
    ...
    t3 <- system.time(run_pca_asym(data2, k))["elapsed"]
    t4 <- system.time(run_pca_em_wd(data2, k))["elapsed"]
    tibble::tibble(
      ncomp  = k,
      method = c("pca_full","pca_pSVD","pca_asym","pca_em_wd"),
      time_s = c(t1, t2, t3, t4),
      mem    = NA_real_
    )
  }
}
...
speed_tbl <- bench_res |>
  ...
  mutate(speedup_pSVD = `pca_full` / `pca_pSVD`,
         speedup_asym = `pca_full` / `pca_asym`,
         speedup_em_wd = `pca_full` / `pca_em_wd`) |>
  arrange(ncomp)
...
cmp_one <- function(k) {
  ...
  a <- run_pca_asym(data2, k)
  e <- run_pca_em_wd(data2, k)
  tibble::tibble(
    ncomp = rep(k, 3*k),
    method = rep(c("pca_pSVD","pca_asym","pca_em_wd"), each = k),
    pc = rep(seq_len(k), times = 3),
    abs_diff = c(abs(f$prop_expl_var$X[1:k] - p$prop_expl_var$X[1:k]),
                 abs(f$prop_expl_var$X[1:k] - a$prop_expl_var$X[1:k]),
                 abs(f$prop_expl_var$X[1:k] - e$prop_expl_var$X[1:k]))
  )
}
